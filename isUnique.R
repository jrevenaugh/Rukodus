# isUnique
#
# Given a partial puzzle, isUnique determines if the solution is unique.  It
# returns TRUE for unique puzzles and FALSE for non-unique and unsolvable
# puzzles.  Uses has solution to evaluate multiple solution pathways.
...
isUnique <- function(Puzzle) {
  ...
  while (iGrid) {
    ...
    if (nC > 1) {
      ...
      tPuzzle <- Puzzle
      for (k in 1:nC) {
        tPuzzle[rc[1],rc[2]] <- candidates[k]
        if (hasSolution(tPuzzle)$yesno) {
          nSolutions <- nSolutions + 1
          yesNo <- k
        }
        if (nSolutions == 2) break
      }
      if (nSolutions > 1) {
        return(FALSE)
